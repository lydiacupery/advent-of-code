  ...
  const total = characterLines.reduce((acc, characterLine, yIndex) => {
    ...
    return (
      ...
      characterLine.reduce((runningSubTotal, character, xIndex) => {
	    ...
	    const topLeftMatch = characterLines[yIndex - 1][xIndex - 1].isNumber
	    const topMatch = characterLines[yIndex - 1][xIndex].isNumber
	    const topRightMatch = characterLines[yIndex - 1][xIndex + 1].isNumber
            if (
             topLeftMatch && topMatch && topRightMatch ||  
            ) {
		numbers.push()
            }
          }
